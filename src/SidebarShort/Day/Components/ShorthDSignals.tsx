...
// import * as ShortDSignalsLogic from '../Logic/ShortDSignalsLogic';

// Interfaces are now imported from Types

const db = getFirestore();

export default function ShorthDSignals() {
    const [selectedDate, setSelectedDate] = useState<string>('');
    const [favoriteStocks, setFavoriteStocks] = useState<DSignalsTypes.FavoriteStock[]>([]);
    const [momentumResults, setMomentumResults] = useState<DSignalsTypes.MomentumResult[]>([]);
    ...
    // נתונים מקומיים
    const [localData, setLocalData] = useState<DSignalsTypes.LocalStockData | null>(null);
    ...
    const loadFavoriteStocks = async () => {
        try {
            ...
            let stocksData: DSignalsTypes.FavoriteStock[] = [];
    ...
    // חישוב ציון מומנטום לשורט בלבד (1-100) - הפוך מ-Long
    const calculateShortMomentumScore = (crossoverType: 'Bullish' | 'Bearish' | 'None', macdHistogram: number): number => {
            ...
            for (let i = 0; i < favoriteStocks.length; i++) {
                ...
                try {
                    ...
                    if (historicalData.length < 35) {
                        ...
                        // Continue with available data instead of skipping completely
                        if (historicalData.length < 12) {
                            console.warn(`⚠️ Too little data for ${stock.symbol}, skipping analysis`);
                            continue;
                        }
                    }
                    ...
                    // חישוב ציון לשורט בלבד
                    const shortMomentumScore = calculateShortMomentumScore(crossoverType, macdHistogram);
                    ...
                    const result: DSignalsTypes.MomentumResult = {
                        ...
                        crossoverType,
                        macdHistogram,
                        LongMomentumScore: shortMomentumScore, // נשתמש באותו שדה אבל עם לוגיקת Short
                    ...
                    console.log(`✅ ${stock.symbol} ShortMomentumScore analysis:`, {
                        crossover: crossoverType,
                        macdHistogram: macdHistogram.toFixed(4),
                        score: shortMomentumScore
                    });
    ...
    const saveToFirebase = async () => {
        ...
        try {
            // עדכון קולקציית short_daily_relative-strength עם ציוני מומנטום
            ...
            if (existingData?.results) {
                    const updatedResults = existingData.results.map((stock: any) => {
                        ...
                        if (momentumResult) {
                            return {
                                ...stock,
                                ShortMomentumScore: momentumResult.LongMomentumScore, // נשתמש באותו שדה אבל עם שם Short
                ...
                console.log(`✅ Updated short_daily_relative-strength document ${shortRelativeStrengthDocId} with momentum scores`);
                ...
                alert(`תוצאות ניתוח מומנטום לשורט עודכנו ב-short_daily_relative-strength בהצלחה! 📊\n\n` +
                      `📅 תאריך: ${selectedDate}\n` +
    ...
    return (
        <Box sx={{ padding: 3, maxWidth: 1400, margin: '0 auto' }}>
            ...
            <Typography variant="h4" sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 1 }}>
                <SignalCellularAlt color="error" />
                ShorthDSignals - תחנה 4: מומנטום ראשוני לשורט בלבד
            </Typography>

            {/* הסבר */}
            <Alert severity="info" sx={{ mb: 3 }}>
                📉 <strong>תחנה 4: מדדי תנודתיות וחוזק לשורט:</strong><br/>
                🔹 <strong>SMA Crossover:</strong> חצייה של SMA3 מול SMA12 (Bear = טוב לשורט)<br/>
                🔹 <strong>MACD Histogram:</strong> אישור מומנטום (&lt;0 יורד = טוב לשורט)<br/>
                🔹 <strong>ShortMomentumScore:</strong> ציון לשורט בלבד (1-100)
            </Alert>
                ...
                <CardContent>
                    ...
                    <Stack spacing={3}>
                        ...
                        <Stack direction="row" spacing={2}>
                            ...
                            >
                                📉 נתח מומנטום לשורט ({favoriteStocks.length} מניות) - 📁 מקומי בלבד
                            </Button>
